...
class DataIntegrationIDE:
    ...
    def log_message(self, topic, key, value, message_type="UNKNOWN", offset=None, partition=None):
        ...
        try:
            ...
            # Update log viewer if it exists and is still open
            if (hasattr(self, 'log_viewer') and 
                'window' in self.log_viewer and 
                self.log_viewer['window'].winfo_exists()):
                try:
                    ...
                    # If widgets are destroyed, remove the log viewer reference
                    delattr(self, 'log_viewer')
    ...
    def create_log_viewer(self):
        ...
        def on_window_close():
            """Handle log viewer window closing"""
    ...
    def __init__(self):
        ...
        self.load_config()
        self.setup_ui()
        # Add this after creating the monitoring tab
    ...
    def refresh_monitoring(self):
        ...
        try:
            ...
            # Dictionary to store rule details
            deployed_rules = {}
            ...
            for mapping_file in mapping_files:
                try:
                    with open(mapping_file, 'r') as f:
                        ...
                        # Store rule details
                        deployed_rules[rule_name] = {
                            'source_type': mapping['source']['type'],
                            'source_table': mapping['source']['table'],
                            'target_label': mapping['target']['label'],
                            'kafka_topic': f"{self.kafka_entries['topic_prefix'].get()}_{rule_name}"
                        }
                        
                        # Insert into rules tree
                        self.rules_tree.insert('', 'end', values=(
                            rule_name,
                            f"{mapping['source']['type']} - {mapping['source']['table']}",
                            f"Neo4j - {mapping['target']['label']}",
                            f"{self.kafka_entries['topic_prefix'].get()}_{rule_name}",
                            'Deployed'
                        ))
                        
                except Exception as e:
                    self.logger.error(f"Failed to load mapping file {mapping_file}: {str(e)}")
                    continue
            
            # Load synchronization status from database
            conn = sqlite3.connect('monitoring.db')
            c = conn.cursor()
            
            try:
                # First try to get latest status for each rule
                c.execute('''SELECT rule_name, MAX(timestamp) as last_update
                            FROM integration_status 
                            GROUP BY rule_name''')
                latest_updates = dict(c.fetchall())
                
                for rule_name in deployed_rules:
                    if rule_name in latest_updates:
                        # Get detailed status for this timestamp
                        c.execute('''SELECT timestamp, rule_name, records_processed, 
                                    success_count, error_count 
                                    FROM integration_status 
                                    WHERE rule_name = ? AND timestamp = ?''',
                                (rule_name, latest_updates[rule_name]))
                        status = c.fetchone()
                        if status:
                            self.status_tree.insert('', 'end', values=status)
                    else:
                        # Insert initial status for rules without any history
                ...
                if status_rows:
                    ...
                    for row in status_rows:
                        if row[1] in deployed_rules:  # Only show status for deployed rules
                            self.status_tree.insert('', 'end', values=row)
                        
            except sqlite3.Error as e:
                self.logger.error(f"Database error: {str(e)}")
                # Add placeholder status if database query fails
                for rule_name in deployed_rules:
                    self.status_tree.insert('', 'end', values=(
                        datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
                        rule_name,
    ...
    def show_kafka_status(self):
        ...
        if not hasattr(self, 'status_window') or not self.status_window.winfo_exists():
            ...
            self.status_window.title("Kafka Services Status")
            self.status_window.geometry("400x300")
            
            # Create frames dictionary to store service frames
            self.service_frames = {}
            ...
            # Create frames for each service
            for service_name in services:
                frame = ttk.Frame(self.status_window)
                frame.pack(fill='x', padx=20, pady=5)
                self.service_frames[service_name] = {
                    'frame': frame,
                    'name_label': ttk.Label(frame, text=f"{service_name}:"),
                    'status_label': ttk.Label(frame),
                    'status_canvas': tk.Canvas(frame, width=12, height=12),
                    'url_label': ttk.Label(frame, font=('TkDefaultFont', 8)),
                    'desc_label': ttk.Label(frame, font=('TkDefaultFont', 8, 'italic'))
                }
                
                # Pack the widgets
                self.service_frames[service_name]['name_label'].pack(side='left')
                self.service_frames[service_name]['status_canvas'].pack(side='right')
                self.service_frames[service_name]['status_label'].pack(side='right', padx=5)
                self.service_frames[service_name]['url_label'].pack(pady=(0,2))
                self.service_frames[service_name]['desc_label'].pack()
            
            # Add refresh button
            ttk.Button(self.status_window, text="Refresh Status", 
                    command=self.refresh_status).pack(pady=10)
            
            # Create warning label
            self.warning_label = ttk.Label(self.status_window, 
                                        wraplength=350,
                                        foreground='red')
            self.warning_label.pack(pady=10, padx=20)
        
        # Update the status display
        self.update_kafka_status_display()

    def update_kafka_status_display(self):
        """Update the status display in the existing window"""
        ...
        for service_name, details in services.items():
            frame_data = self.service_frames[service_name]
            
            # Update status indicator
            color = "#2ECC71" if details['running'] else "#E74C3C"
            frame_data['status_canvas'].delete("all")
            frame_data['status_canvas'].create_oval(2, 2, 10, 10, fill=color, outline=color)
            
            # Update status text
            frame_data['status_label'].config(
                text='Running' if details['running'] else 'Not Running'
            )
            
            # Update URL and description
            frame_data['url_label'].config(text=f"URL: {details['url']}")
            frame_data['desc_label'].config(text=f"({details['description']})")
    ...
    def create_mapping_rules(self):
        ...
        # Modify the rule selection frame
        ...
        # Button frame for rule management
        button_frame = ttk.Frame(rule_select_frame)
        ...
        ttk.Button(button_frame, text="Update", 
                command=self.update_mapping).pack(side='left', padx=2)
        ttk.Button(button_frame, text="Show", 
                command=self.show_mapping_content).pack(side='left', padx=2)
    ...
    def update_mapping(self):
        """Update the current mapping rule"""
        current = self.mapping_rule_var.get()
        if not current:
            messagebox.showwarning("Warning", "Please select a mapping rule to update")
            return
            
        try:
            # Save current mapping state
            self.save_current_mapping()
            messagebox.showinfo("Success", f"Mapping rule '{current}' updated successfully")
            
        except Exception as e:
            self.logger.error(f"Failed to update mapping rule: {str(e)}")
            messagebox.showerror("Error", "Failed to update mapping rule")

    def show_mapping_content(self):
        """Show the content of the current mapping rule"""
        current = self.mapping_rule_var.get()
        if not current:
            messagebox.showwarning("Warning", "Please select a mapping rule to show")
            return
        ...
        try:
            ...
            if os.path.exists(mapping_file):
                ...
                viewer = tk.Toplevel(self.root)
                viewer.title(f"Mapping Rule: {current}")
                viewer.geometry("600x800")
                ...
                content = "Mapping Rule Details:\n\n"
                content += f"Rule Name: {current}\n\n"
                content += "Source:\n"
                content += f"  Type: {mapping['source']['type']}\n"
                content += f"  Table: {mapping['source']['table']}\n\n"
                content += "Target:\n"
                content += f"  Type: {mapping['target']['type']}\n"
                content += f"  Label: {mapping['target']['label']}\n\n"
                content += "Column Mappings:\n"
                for source, target in mapping['columns'].items():
                    content += f"  {source} → {target}\n"
                
                # Also add the raw JSON for reference
                content += "\nRaw JSON:\n"
                ...
                # Add close button
                ttk.Button(viewer, text="Close", 
                        command=viewer.destroy).pack(pady=10)
                
            else:
                messagebox.showerror("Error", f"Mapping file for '{current}' not found")
                
        except Exception as e:
            self.logger.error(f"Failed to show mapping content: {str(e)}")
            messagebox.showerror("Error", "Failed to show mapping content")
    ...
    def show_rule_topics(self, rule_name):
        ...
        try:
            ...
            def refresh_data(tree=topics_tree, status_frame=status_frame):
                ...
                try:
                    ...
                    if sync_status:
                        ...
                        ttk.Label(status_frame, 
                                text=f"Last Sync: {sync_status['last_sync']}\n"
            ...
            # Add buttons with proper references to refresh_data
            ttk.Button(button_frame, text="Refresh", 
                    command=lambda: refresh_data(topics_tree, status_frame)).pack(side='left', padx=5)
            ttk.Button(button_frame, text="View Messages",
            ...
            # Add start/stop background processing buttons
            ttk.Button(button_frame, text="Start Background Processing",
                    command=lambda: self.start_background_threads(rule_name)).pack(side='left', padx=5)
            ttk.Button(button_frame, text="Stop Background Processing",
                    command=lambda: self.stop_background_threads(rule_name)).pack(side='left', padx=5)

            # Initial data load
            refresh_data(topics_tree, status_frame)
        ...
        try:
            ...
            with sqlite3.connect('monitoring.db') as conn:
                ...
                c.execute('''INSERT INTO integration_status 
                            ...
                            success_count, error_count, status)
                            VALUES (?, datetime('now'), ?, ?, ?, ?, ?)''',
                        (status_id, rule_name, processed, success, errors, 
                        'ERROR' if errors > 0 else 'SUCCESS'))
                conn.commit()
                
            # Refresh the monitoring display if we're on the monitoring tab
            current_tab = self.notebook.select()
            if self.notebook.index(current_tab) == self.steps.index("Monitoring"):
                self.refresh_monitoring()
